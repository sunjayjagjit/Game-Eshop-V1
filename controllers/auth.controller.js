...
async function signup(req, res, next) {
  ...
  try {
    const existsAlready = await user.existEmailAlready();
    if (existsAlready) {
      sessionFlash.flashDataToSession(
        req,
        {
...
async function login(req, res, next) {
  ...
  const sessionErrorData = {
    errorMessage: "Invalid credentials - please check your email adn password",
    email: user.email,
  ...
  //password matching
  const passwordIsCorrect = await user.comparePasswordUser(
